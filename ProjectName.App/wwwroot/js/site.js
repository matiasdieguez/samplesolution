...
function checkWindow(){
    var div1 = $("#section-header");
    var div2 = $("#section-about");
    var div3 = $("#section-help");
    var div4 = $("#index");


    if ($(window).width() > 780) {
        if (div1.css('display') == 'block') {
            $("#mark-nav-i").addClass("mark-nav");
            document.getElementById("index").style.color = "#ffffff";
            document.getElementById("index").style.backgroundColor = "#0346fa";
        }
        if (div2.css('display') == 'block') {

            $("#mark-nav-a").addClass("mark-nav");
            document.getElementById("about").style.color = "#ffffff";
            document.getElementById("about").style.backgroundColor = "#0346fa";
        }
        if (div3.css('display') == 'block') {
            $("#mark-nav-h").addClass("mark-nav");
            document.getElementById("help").style.color = "#ffffff";
            document.getElementById("help").style.backgroundColor = "#0346fa";
        }

    } else {
        if (div1.css('display') == 'block') {
            $("#mark-nav-i").removeClass("mark-nav");
            document.getElementById("index").style.color = "#004a8c";
            document.getElementById("index").style.backgroundColor = "#ffffff";

        }
        if (div2.css('display') == 'block') {

            $("#mark-nav-a").removeClass("mark-nav");
            document.getElementById("about").style.color = "#004a8c";
            document.getElementById("about").style.backgroundColor = "#ffffff";
        }
        if (div3.css('display') == 'block') {
            $("#mark-nav-h").removeClass("mark-nav");
            document.getElementById("help").style.color = "#004a8c";
...
$(document).ready(function() {
    $(".block-text-footer").find('.title-footer').click(function() {
        var $thiselements = $(this).parent().find('.footer-drop-down');
        if ($(window).width() < 768) {
            for (i = 0; i < $thiselements.length; i++) {
                $($thiselements[i]).slideToggle('10');
            }
        }
...
$(window).resize(function() {
    var $thiselements = $(document).find('.footer-drop-down');
    if ($(window).width() > 780) {
        for (i = 0; i < $thiselements.length; i++) {
            $($thiselements[i]).slideDown('10');
        }
    }
    else{
        for (i = 0; i < $thiselements.length; i++) {
            $($thiselements[i]).slideUp('10');
        }
    }
});
...
$('.questions').children().click(function() {
    ...
    $(this).children('.answer').slideToggle('100');
});
...
var $RequireContent = $('.requirement');
var $AboutText = $('.text-about');
var $AboutImg = $('.about-img');
var $AboutButton = $('.button-about');
var $About = $('#section-about');
var $StepsResponsive = $('#steps');
...
$About.waypoint(function(direction) {
    if ($(window).width() > 800) {
        if (direction === "down") {
            $AboutText.addClass('wp-text-about');
            $AboutButton.addClass('wp-button-about').delay(1000);
        } else {
            $AboutText.removeClass('wp-text-about');
            $AboutButton.removeClass('wp-button-about').delay(1000);
        }
    } else {
...
$StepsResponsive.waypoint(function(direction) {
    if ($(window).width() > 800) {
        if (direction === "down") {
            //     yendoParaAbajo();
            timeline();
...
function timeline() {
    $("#timeline-ol").animate({ opacity: 1 });
    $(".timeline").animate({ opacity: 1 });
};

function timelineUp() {
    $("#timeline-ol").animate({ opacity: 0 });
    $(".timeline").animate({ opacity: 0 });
};
...
$(document).ready(function() {
    $("#btn-user").click(function() {
        ...
        $("#btn-user").hide();
        $("#btn-back-login").show();
        $(".content-login").addClass("content-login-client");
    });
    $("#btn-back-login").click(function() {
        ...
        $("#btn-user").show();
        $("#btn-back-login").hide();
        $(".content-login").removeClass("content-login-client");
    });
});

$ (document).ready(function(){
    $("#mydata").click(function() {
        $("#mydatadp").slideToggle();
        $("#mycreditdp").hide();
        $("#requestcreddp").hide();
    });
    $("#mycredit").click(function() {
        $("#mycreditdp").slideToggle();
        $("#requestcreddp").hide();
        $("#mydatadp").hide();
    });
    $("#requestcred").click(function() {
        $("#requestcreddp").slideToggle();
        $("#mycreditdp").hide();
        $("#mydatadp").hide();
    });
});
